...
func DequeueIngestion(key string, fullsync bool) {
	...
	if objType == utils.Service {
		...
		if found {
			...
			defer func() {
				objects.InfraSettingL7Lister().RemoveGwSvcToInfraSettingMapping(namespace + "/" + name)
			}()
			model_name := lib.GetModelName(lib.GetTenantFromInfraSettingForGwSvc(namespace, name), lib.Encode(lib.GetNamePrefix()+namespace+"-"+name, lib.L4VS))
		...
		if gatewayFound {
			for _, gatewayKey := range gateways {
				...
				if isGatewayDelete(gatewayKey, key) {
					defer func() {
						objects.InfraSettingL7Lister().RemoveGwSvcToInfraSettingMapping(namespace + "/" + gwName)
					}()
					// Check if a model corresponding to the gateway exists or not in memory.
					modelName := lib.GetModelName(lib.GetTenantFromInfraSettingForGwSvc(namespace, gwName), lib.Encode(lib.GetNamePrefix()+namespace+"-"+gwName, lib.ADVANCED_L4))
...
func handleL4SharedVipService(namespacedVipKey, key string, fullsync bool) {
	...
	modelName := lib.GetModelName(lib.GetTenantFromInfraSettingForGwSvc(namespace, name), lib.Encode(lib.GetNamePrefix()+strings.ReplaceAll(namespacedVipKey, "/", "-"), lib.ADVANCED_L4))
