...
func (rest *RestOperations) StatusUpdateForVS(restMethod utils.RestMethod, vsCacheObj *avicache.AviVsCache, key string) {
	...
	case lib.HTTPRouteChildVS:
		...
		utils.AviLog.Infof("key: %s Publishing HTTPRoute status to status queue, options: %v", serviceMetadataObj.HTTPRoute, utils.Stringify(statusOption))
	...
	if ok {
		...
		if found {
			...
			case lib.HTTPRouteChildVS:
				...
				utils.AviLog.Infof("key: %s Publishing HTTPRoute status to status queue, options: %v", vsCacheObj.ServiceMetadataObj.HTTPRoute, utils.Stringify(statusOption))
